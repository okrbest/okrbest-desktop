...
1. Copy the files from this directory (`resources/windows/gpo`) to `C:\Windows\PolicyDefinitions` - the file `okrbest.admx` should go in `C:\Windows\PolicyDefinitions` and the file `okrbest.adml` should go in `C:\Windows\PolicyDefinitions\en-US`.
2. Press `Win` + `R` to open the "Run" box
3. Type `gpedit.msc` to open the Group Policy Editor
4. Select "Administrative Templates" => "OKR Best" - either in "Computer Configuration" or "User Configuration" it shouldn't matter.
...
| Value Name | Value                         |
|------------|-------------------------------|
| Team       | <https://team.okrbest.com>    |

8. Now if you open your OKR Best desktop application you should be able to see the server in the server dropdown
