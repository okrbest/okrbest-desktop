...
Otherwise, first locate the extracted directory in your desired directory (e.g. `/opt/okrbest-desktop`) and follow the steps below. (This directory also contains this README here.)

### Desktop launcher

Execute the script file to create a `OKRBest.desktop` file.

```
/opt/okrbest-desktop/create_desktop_file.sh
```
...
```
mv OKRBest.desktop ~/.local/share/applications/
```
...
```sh
# assuming that /opt/okrbest-desktop/okrbest-desktop is the executable file.
export PATH=$PATH:/opt/okrbest-desktop
```

Alternatively, you can also create a symbolic link for the application.

```sh
sudo ln -s /opt/okrbest-desktop/okrbest-desktop /usr/local/bin/
```
...
See [contributing guidelines](https://github.com/okrbest/okrbest-desktop/blob/master/CONTRIBUTING.md) for reporting bugs, features or submitting pull requests.
